...
const PostItem = ({
  ...
  return (
    <Post>
          ...
          <PostLink link={item.link} className="postLink">
            {item.title}
          </PostLink>
          {/* If the post has categories, render the categories */}
          {/* {item.categories && <PostCategories categories={categories} />} */}

          {/* If the post has secondary title */}
          {item.acf.secondary_title && 
              <h4 className="secondary-title">{item.acf.secondary_title}</h4>
          }


          {/* If the post has excerpt */}
          {item.acf.short_description && <PostItemDiv className="excerpt" dangerouslySetInnerHTML={{ __html: item.acf.short_description }} />}

          {/* If the post has sector */}
          {/* {allCategories && 
            <ul>
              {categories.map((cat) => {
                return(
          ...
          {item.acf.test_server_url && item.acf.live_url &&
          <div className="links">
            <a href={item.acf.test_server_url}>Internal</a>
            <a href={item.acf.live_url}>Live</a>
          </div>
          }
        {/*
